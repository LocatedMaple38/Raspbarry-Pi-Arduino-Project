...
void setup(){
  ...
  for(int byte_idx = 0; byte_idx < strlen(message); byte_idx++){
    ...
    // sets the cursor to the char that is being transmited
    // sets sets the courser to line 2 (line one stars at '0')
    // and puts 8 spaces to "clear" the second ln then sets
    // cursor off
    lcd_1.noCursor();
    lcd_1.setCursor(0, 1);
    lcd_1.print("       ");
    ...
    // loops thru each bit of the byte in the letter most 
    // sanifacent bit first
    for(int bit_idx = 0; bit_idx < 8; bit_idx++){
      ...
      // sets the cursror to ln 2 of the desplay then prints 
      // a "1" or a "0" whan dun sets the couser to the char
      // that is being transmited
      lcd_1.noCursor();
